...
select u.first_name,u.last_name,t.Tech_Name from user_details u join Tech_Stack t
on u.id=t.Creator_User where t.Tech_Name='Java';

-- 2- find all mentors and ideations have java technology
SELECT m.Id, m.Name, mi.Id, ts.Tech_Name
FROM Mentor m JOIN Mentor_Ideation_Map mi ON m.Id = mi.Mentor_Id
JOIN Tech_Stack ts ON mi.Id = ts.Id
WHERE ts.Tech_Name = 'Java';

-- 3- find name of candidate which did not assign technology
select u.first_name, u.last_name, ts.Tech_Name from user_details u join tech_Stack ts
on u.id=ts.Creator_User where ts.Tech_Name='';

-- 4- find name of cnadidate which is not submit documents
select u.first_name,u.last_name,c.Status from user_details u 
join candidate_documents c on u.id=c.Candidate_Id where c.Status='Pending';

-- 5- find name of candidate which is not submit bank details
select u.first_name,u.last_name,c.Candidate_Id,cbd.Account_Number from user_details u join candidate_documents c
on u.id=c.Candidate_Id join candidate_bank_details cbd on 
